...
@Component
public class Disccode extends RouteBuilder {
    @Value("${PGP.EMAIL}")
    ...
    public void configure() throws Exception {
        ...
        String fileName="AUSDISCCD_"+date;
        final List<String> columnHeaders = Arrays.asList("dcode","dperct","sdate","edate","rgcd","raction");
        String numRow=((Long)repo.getNumRowsDiscCode()).toString();

        CsvDataFormat csv = new CsvDataFormat();
        csv.setHeader(columnHeaders);

        from("cron:disccode?schedule=0+20+13+*+*+?")
                .marshal(csv)
                .to(filePath + "processed/?FileExist=Append&fileName=" + fileName+"_"+numRow+".csv")
                .setHeader("sftpPath", constant(filePath + fileName + "_" + numRow + ".csv")) // Set sftpPath header
                .to("sql:select code,ROUND(dperct,4) AS dperct,TO_CHAR(DATE(sdate),'MM/DD/YYYY') AS sdate,TO_CHAR(DATE(edate),'MM/DD/YYYY') AS edate,reg as rgcd,(select 'R' as raction) from disccode WHERE CURRENT_DATE BETWEEN DATE(sdate) and DATE(edate) and reg = 'US'?maxMessagesPerPoll=1000&outputType=StreamList&batch=true")
                .onCompletion()
                .onCompleteOnly()
