...
@Component
public class Trkcodf extends RouteBuilder {
    ...
    public void configure() throws Exception {
        ...
        String fileName="AUSTRKCODF_"+date;
        final List<String> columnHeaders = Arrays.asList("tfdscd","trakcode","tctype","tsubtyp","tsoldto","tstrdat","tstpdat","tcqty1","tcqty2","tcqty3","tcqty4","tcqty5","tcqty6","tcqty7","tcamt1","tcamt2","tcamt3","tcamt4","tcamt5","tcamt6","tcamt7","totord","tterm1","tterm2","tterm3","tterm4","tterm5","tterm7","tterm6","raction");
        String numRow=((Long)repo.getNumRowstrkcodf()).toString();

        CsvDataFormat csv = new CsvDataFormat();
        csv.setHeader(columnHeaders);

        from("cron:trkcodf?schedule=0+08+17+*+*+?")
                .marshal(csv)
                .to(filePath + "processed/?FileExist=Append&fileName=" + fileName+"_"+numRow+".csv")
                .to("sql:select tfdscd,trakcode,tctype,tsubtyp,tsoldto,TO_CHAR(DATE(tstrdat),'MM/DD/YYYY') AS tstrdat,TO_CHAR(DATE(tstpdat),'MM/DD/YYYY') AS tstpdat,tcqty1,tcqty2,tcqty3,tcqty4,tcqty5,tcqty6,tcqty7,ROUND(tcamt1,2) AS tcamt1,ROUND(tcamt2,2) AS tcamt2,ROUND(tcamt3,2) AS tcamt3,ROUND(tcamt4,2) AS tcamt4,ROUND(tcamt5,2) AS tcamt5,ROUND(tcamt6,2) AS tcamt6,ROUND(tcamt7,2) AS tcamt7,totord,tterm1,tterm2,tterm3,tterm4,tterm5,tterm7,tterm6,(select 'R' as raction) from trkcodf WHERE (DATE(updated_date) = CURRENT_DATE OR DATE(updated_date) = (CURRENT_DATE - INTERVAL '1 day')) and tfdscd='US'?maxMessagesPerPoll=1000&outputType=StreamList&batch=true")
                .onCompletion()
                .onCompleteOnly()
                .process(exchange -> {
