...
@Component
public class Trkcodmm extends RouteBuilder {
    ...
    public void configure() throws Exception {
        ...
        String fileName="AUSTCODMM_"+date;
        final List<String> columnHeaders = Arrays.asList("mfdscd","mrakcode","mctype","msubtyp","msoldto","mstrdat","mstpdat","mcqty1","mcqty2","mcqty3","mcqty4","mcqty5","mcqty6","mcqty7","motord","mterm1","mterm2","mterm3","mterm4","mterm5","mterm6","mterm7","mmaxpl","mfullp","mrdcyn","mvendor","raction");
        String numRow=((Long)repo.getNumRowsTrkcodmm()).toString();

        CsvDataFormat csv = new CsvDataFormat();
        csv.setHeader(columnHeaders);

        from("cron:trkcodmm?schedule=0+36+16+*+*+?")
                .marshal(csv)
                .to(filePath + "processed/?FileExist=Append&fileName=" + fileName+"_"+numRow+".csv")
                .to("sql:select mfdscd,mrakcode,mctype,msubtyp,msoldto,TO_CHAR(DATE(mstrdat),'MM/DD/YYYY') AS mstrdat,TO_CHAR(DATE(mstpdat),'MM/DD/YYYY') AS mstpdat,mcqty1,mcqty2,mcqty3,mcqty4,mcqty5,mcqty6,mcqty7,motord,mterm1,mterm2,mterm3,mterm4,mterm5,mterm6,mterm7,mmaxpl,mfullp,mrdcyn,mvendor,(select 'R' as record_action) from trkcodmm WHERE (DATE(updated_date) = CURRENT_DATE OR DATE(updated_date) = (CURRENT_DATE - INTERVAL '1 day')) and mfdscd='US'?maxMessagesPerPoll=1000&outputType=StreamList&batch=true")
                .onCompletion()
                .onCompleteOnly()
                .process(exchange -> {
