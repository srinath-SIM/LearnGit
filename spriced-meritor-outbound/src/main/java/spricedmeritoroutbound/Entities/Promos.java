...
@Component
public class Promos extends RouteBuilder {
    ...
    public void configure() throws Exception {
        ...
        String fileName="AUSPROMOS_"+date;
        final List<String> columnHeaders = Arrays.asList("ptrack","pcust","pref","pstart","pstop","ppart","pqty1","pqty2","pqty3","pqty4","pqty5","pqty6","pqty7","pprc1","pprc2","pprc3","pprc4","pprc5","pprc6","pprc7","pdsc1","pdsc2","pdsc3","pdsc4","pdsc5","pdsc6","pdsc7","pdate","puser","prfds","trkqty","raction");
        String numRow=((Long)repo.getNumRowsPromos()).toString();

        CsvDataFormat csv = new CsvDataFormat();
        csv.setHeader(columnHeaders);

        from("cron:promos?schedule=0+30+13+*+*+?")
                .marshal(csv)
                .to(filePath + "processed/?FileExist=Append&fileName=" + fileName+"_"+numRow+".csv")
                .to("sql:select ptrack,pcust,pref,TO_CHAR(DATE(pstart),'MM/DD/YYYY') AS pstart,TO_CHAR(DATE(pstop),'MM/DD/YYYY') AS pstop,ppart,pqty1::BIGINT,pqty2::BIGINT,pqty3::BIGINT,pqty4::BIGINT,pqty5::BIGINT,pqty6::BIGINT,pqty7::BIGINT,ROUND(pprc1,2) AS pprc1,ROUND(pprc2,2) AS pprc2,ROUND(pprc3,2) AS pprc3,ROUND(pprc4,2) AS pprc4,ROUND(pprc5,2)    AS pprc5,ROUND(pprc6,2) AS pprc6,ROUND(pprc7,2) AS pprc7,ROUND(pdsc1,4) AS pdsc1,ROUND(pdsc2,4) AS pdsc2,ROUND(pdsc3,4) AS pdsc3,ROUND(pdsc4,4) AS pdsc4,ROUND(pdsc5,4) AS pdsc5,ROUND(pdsc6,4) AS pdsc6,ROUND(pdsc7,4) AS pdsc7,TO_CHAR(DATE(pdate),'MM/DD/YYYY') AS pdate,puser,prfds,trkqty,(select 'R' as raction) from promos WHERE (DATE(updated_date)=CURRENT_DATE OR DATE(updated_date) = (CURRENT_DATE - INTERVAL '1 day')) and prfds='US'?maxMessagesPerPoll=1000&outputType=StreamList&batch=true")
                .onCompletion()
                .onCompleteOnly()
                .process(exchange -> {
