...
@Component
public class Partdisc extends RouteBuilder {
    @Value("${PGP.EMAIL}")
    ...
    public void configure() throws Exception {
        ...
        String fileName="AUSPRTDISC_"+date;
        final List<String> columnHeaders = Arrays.asList("pfds","psoldt","ppcode","pdisc","pstrdt","penddt","raction");
        String numRow=((Long)repo.getNumRowsPartDisc()).toString();

        CsvDataFormat csv = new CsvDataFormat();
        csv.setHeader(columnHeaders);

        from("cron:partdisc?schedule=0+20+13+*+*+?")
                .marshal(csv)
                .to(filePath + "processed/?FileExist=Append&fileName=" + fileName+"_"+numRow+".csv")
                .to("sql:select pfds,psoldt,ppcode,pdisc,TO_CHAR(DATE(pstrdt),'MM/DD/YYYY') AS pstrdt,TO_CHAR(DATE(penddt),'MM/DD/YYYY') AS penddt,(select 'R' as record_action) from partdisc WHERE CURRENT_DATE BETWEEN DATE(pstrdt) and DATE(penddt)and pfds='US'?maxMessagesPerPoll=1000&outputType=StreamList&batch=true")
                .onCompletion()
                .onCompleteOnly()
                .process(exchange -> {
