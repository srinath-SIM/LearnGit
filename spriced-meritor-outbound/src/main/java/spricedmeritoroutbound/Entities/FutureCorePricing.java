...
@Component
public class FutureCorePricing extends RouteBuilder {
    ...
    public void configure() throws Exception {
        ...
        String fileName="OCACOREPRC_"+date;
        final List<String> columnHeaders = Arrays.asList("cpn","coreg","fcoreg","fcpn","fcc","updt","stat","effdt","ca","raction");
        String numRow=((Long)repo.getNumRowsFcp()).toString();

        CsvDataFormat csv = new CsvDataFormat();
        csv.setHeader(columnHeaders);

        from("cron:tab?schedule=0+20+13+*+*+?")
                .marshal(csv)
                .to(filePath + "processed/?FileExist=Append&fileName=" + fileName+"_"+numRow+".csv")
                .to("sql:select cpn,coreg,fcoreg,fcpn,fcc,updt,stat,TO_CHAR(DATE(effdt),'MM/DD/YYYY') AS effdt,regcd,(SELECT 'R' AS raction) from future_core_planning where DATE(effdt)=current_date?maxMessagesPerPoll=1000&outputType=StreamList&batch=true")
                .onCompletion()
                    .onCompleteOnly()
                    .process(exchange -> {
...
//                      long milliseconds = processingDuration.toMillis();
                    ...
                    .to(String.format("sftp://"+user+"@"+host+":"+sftpPort+destination+"?fileName="+fileName+"_"+numRow+".csv.gz.pgp&privateKeyFile="+sftpPrivateKey+"&privateKeyPassphrase="+passPhrase))
