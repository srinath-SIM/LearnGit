...
@Component
public class Quotefil extends RouteBuilder {
    ...
    public void configure() throws Exception {
        ...
        String fileName="AUSQUOTE_"+date;
        final List<String> columnHeaders = Arrays.asList("qsoldt","qpartno","qcustno","quote","qfds","qfrom","qto","qmail","qrefer","qattn","qmin1","qmin2","qmin3","qmin4","qmin5","qmax1","qmax2","qmax3","qmax4","qmax5","qprc1","qprc2","qprc3","qprc4","qprc5","qdsc1","qdsc2","qdsc3","qdsc4","qdsc5","qlist","qspecl","qprint","qcmt1","qcmt2","qcomqty","raction");
        String numRow=((Long)repo.getNumRowsQuotefil()).toString();

        CsvDataFormat csv = new CsvDataFormat();
        csv.setHeader(columnHeaders);

        from("cron:quotefil?schedule=0+20+13+*+*+?")
                .marshal(csv)
                .to(filePath + "processed/?FileExist=Append&fileName=" + fileName+"_"+numRow+".csv")
                .to("sql:select qsoldt,qpartno,qcustno,quote,qfds,TO_CHAR(DATE(qfrom),'MM/DD/YYYY') AS qfrom,TO_CHAR(DATE(qto),'MM/DD/YYYY') AS qto,qmail,qrefer,qattn,qmin1::BIGINT,qmin2::BIGINT,qmin3::BIGINT,qmin4::BIGINT,qmin5::BIGINT,qmax1::BIGINT,qmax2::BIGINT,qmax3::BIGINT,qmax4::BIGINT,qmax5::BIGINT,ROUND(qprc1,2) AS qprc1,ROUND(qprc2,2) AS qprc2,ROUND(qprc3,2) AS qprc3,ROUND(qprc4,2) AS qprc4,ROUND(qprc5,2) AS qprc5,ROUND(qdsc1,4) AS qdsc1,ROUND(qdsc2,4) AS qdsc2,ROUND(qdsc3,4) AS qdsc3,ROUND(qdsc4,4) AS qdsc4,ROUND   (qdsc5,4) AS qdsc5,ROUND(qlist,2) AS qlist,qspecl,qprint,qcmt1,qcmt2,qcomqty::BIGINT,(select 'R' as raction) from quotefil WHERE (DATE(updated_date) = CURRENT_DATE OR DATE(updated_date) = (CURRENT_DATE - INTERVAL '1 day')) and qfds='US'?maxMessagesPerPoll=1000&outputType=StreamList&batch=true")
                .onCompletion()
                    .onCompleteOnly()
                    .process(exchange -> {
