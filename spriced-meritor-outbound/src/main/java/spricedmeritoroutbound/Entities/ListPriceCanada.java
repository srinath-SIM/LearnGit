...
public class ListPriceCanada extends RouteBuilder {
    ...
    public void configure() throws Exception {
        ...
        from("cron:listPriceCA?schedule=0+20+13+*+*+?")
                .marshal(csv)
                .to(filePath + "processed/?FileExist=Append&fileName=" + fileName+"_"+numRow+".csv")
                .to("sql:SELECT (select 'R' as record_action),(select 'CA' as region),partnum as PART_NO,ROUND(lpriceca,2) AS lprice,TO_CHAR(DATE(fromdat),'MM/DD/YYYY') AS fromdat from list_price WHERE (DATE(updated_date) = CURRENT_DATE OR DATE(updated_date) = (CURRENT_DATE - INTERVAL '1 day')) AND partnum is not null AND lpriceca is not null?maxMessagesPerPoll=1000&outputType=StreamList&batch=true")
                .onCompletion()
                .onCompleteOnly()
                .process(exchange -> {
