...
@Component
public class Prcdsc extends RouteBuilder {
    @Value("${PGP.EMAIL}")
    ...
    public void configure() throws Exception {
        ...
        String fileName="AUSPRCDSC_"+date;
        final List<String> columnHeaders = Arrays.asList("prcsoldt","prcshipt","prcbrnd","prcprod","prcchain","prcdscp","prcstdt","prceddt","prcregcd","raction");
        String numRow=((Long)repo.getNumRowsPrcdsc()).toString();

        CsvDataFormat csv = new CsvDataFormat();
        csv.setHeader(columnHeaders);

        from("cron:prcdsc?schedule=0+20+13+*+*+?")
                .marshal(csv)
                .to(filePath + "processed/?FileExist=Append&fileName=" + fileName+"_"+numRow+".csv")
                .to("sql:select prcsoldt,prcshipt,prcbrnd,prcprod,prcchain,ROUND(prcdscp,2) AS prcdscp,TO_CHAR(DATE(prcstdt),'MM/DD/YYYY') AS prcstdt,TO_CHAR(DATE(prceddt),'MM/DD/YYYY') AS prceddt,prcregcd,(select 'R' as record_action) from prcdsc WHERE CURRENT_DATE BETWEEN DATE(prcstdt) and DATE(prceddt) and prcregcd='US'?maxMessagesPerPoll=1000&outputType=StreamList&batch=true")
                .onCompletion()
                .onCompleteOnly()
                .process(exchange -> {
