...
@Component
public class Prmoprce extends RouteBuilder {
    ...
    public void configure() throws Exception {
        ...
        String fileName="AUSPRMPRCE_"+date;
        final List<String> columnHeaders = Arrays.asList("prmfds","prmcod","prmtrk","prmlev","prmbeg","prmend","prmadd","prmusr","raction");
        String numRow=((Long)repo.getNumRowsPrmoprce()).toString();

        CsvDataFormat csv = new CsvDataFormat();
        csv.setHeader(columnHeaders);

        from("cron:prmoprce?schedule=0+20+13+*+*+?")
                .marshal(csv)
                .to(filePath + "processed/?FileExist=Append&fileName=" + fileName+"_"+numRow+".csv")
                .to("sql:select prmfds,prmcod,prmtrk,prmlev,TO_CHAR(DATE(prmbeg),'MM/DD/YYYY') AS prmbeg,TO_CHAR(DATE(prmend),'MM/DD/YYYY') AS prmend,TO_CHAR(DATE(prmadd),'MM/DD/YYYY') AS prmadd,prmusr,(select 'R' as raction) from prmoprce WHERE (DATE(updated_date) = CURRENT_DATE OR DATE(updated_date) = (CURRENT_DATE - INTERVAL '1 day')) and prmfds='US'?maxMessagesPerPoll=1000&outputType=StreamList&batch=true")
                .onCompletion()
                    .onCompleteOnly()
                    .process(exchange -> {
